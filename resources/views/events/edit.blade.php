...
@section('content')
    ...
    <form method="POST" action="{{ route('events.update', $event->id) }}">
        ...
        <div>
            ...
            <input type="datetime-local" name="start" id="start" value="{{ old('start', $event->start->format('Y-m-d\TH:i')) }}" required>
            @error('start')
                <span class="error">{{ $message }}</span>
            @enderror
        </div>
        <div>
            ...
            <input type="datetime-local" name="end" id="end" value="{{ old('end', $event->end->format('Y-m-d\TH:i')) }}" required>
            @error('end')
                <span class="error">{{ $message }}</span>
            @enderror
        </div>
